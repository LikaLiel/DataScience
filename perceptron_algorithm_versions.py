...
def pegasos_single_step_update(
        ...
        theta_0):
    """
    Updates the classification parameters `theta` and `theta_0` via a single step of the Pegasos algorithm.
...
def classify(feature_matrix, theta, theta_0):
    ...
    classification = np.where(prediction > 0, 1, -1)
...
def extract_words(text):
    ...
    Args:
        a string `text`.
    Returns:
        a list of lower-cased words in the string, where punctuation and digits
    ...
    for text in texts:
        ...
        for word in word_list:
            if word in indices_by_word:
                continue
            if (word in stopword) & remove_stopword:
                continue
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=False):
    ...
    if binarize:
        return (feature_matrix > 0).astype(int)
